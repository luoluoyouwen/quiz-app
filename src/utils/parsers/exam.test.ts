...
// ── Fix: AI normalize preserves answer marker position ──
// AI normalize should NOT move inline answer markers to the front.
// When marker stays in middle position, parser must replace with ____.

describe('choice: AI normalize preserves middle marker position', () => {

  it('keeps marker in middle and replaces with ____ (AI no longer moves to front)', () => {
    // This simulates the CORRECT AI normalize output (after prompt fix):
    // marker stays in its original position, only parentheses normalized
    const input = `二 单选题
...
A.地质灾害    B.森林火灾  C.公共卫生事件   D.群体性事件`;
  ...
  it('handles full-width parens with half-width letter inside (AI normalizes)', () => {
    // AI would normalize （ C ） to (C) while keeping position
    const input = `二 单选题

离心泵启动前（C）灌泵的原因是什么
A.原因一    B.原因二    C.原因三    D.原因四`;
    const result = parseExamDocx(input);
    expect(result.questions[0].content).toContain('____');
    expect(result.questions[0].content).not.toContain('（C）');
    expect(result.questions[0].content).not.toContain('(C)');
  });
});
