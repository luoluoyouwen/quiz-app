...
A. 一取一 B. 二取一 C. 三取二 D. 三取三`;
    ...
    expect(result.questions.length).toBe(1);
    expect(result.questions[0].options?.length).toBe(4);
    expect(result.questions[0].options![0]).toBe('A. 一取一');
    expect(result.questions[0].options![1]).toBe('B. 二取一');
    expect(result.questions[0].options![2]).toBe('C. 三取二');
    expect(result.questions[0].options![3]).toBe('D. 三取三');
  });
...
A、统一管理B、分别管理C、由派遣单位管理`;
    ...
    expect(result.questions[0].options![0]).toBe('A、统一管理');
    expect(result.questions[0].options![1]).toBe('B、分别管理');
    expect(result.questions[0].options![2]).toBe('C、由派遣单位管理');
  });
...
A略高于       B.略低于       C.等于       D.没有规律`;
    ...
    expect(result.questions.length).toBe(1);
    expect(result.questions[0].options?.length).toBe(4);
    expect(result.questions[0].options![0]).toBe('A略高于');
  });
