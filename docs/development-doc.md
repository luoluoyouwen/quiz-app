...
| 原始 URL | 代理 URL | 说明 |
|----------|----------|------|
| `[redacted-supabase-url]/auth/v1/...` | `/api/auth/v1/...` | Auth 请求 |
| `[redacted-supabase-url]/rest/v1/...` | `/api/rest/v1/...` | REST API 请求 |
...
```sql
-- 注册一个工号后执行：
UPDATE profiles SET role='admin' WHERE email='contact@example.invalid';
```
...
| 编号 | 功能 | 说明 | 预期工作量 |
|------|------|------|-----------|
| ~~F-01~~ | ~~题库隔离~~ | ~~不同题库的练习数据（session/answers）完全隔离，不可互访~~ | ✅ 已完成（Dexie + Supabase 均按 bank_id 隔离） |
| ~~F-02~~ | ~~选择题随机选项~~ | ~~每次练习时选项顺序打乱，避免记位置~~ | ✅ 已完成 v1.2.0 |
| ~~F-03~~ | ~~答题后自动下一题~~ | ~~提交正确答案后自动跳转，无需手动点击"下一题"~~ | ✅ 已完成 v1.2.0 |
...
| ~~F-16~~ | ~~随机抽题~~ | ~~从题库中随机抽取 N 题组成练习卷~~ | ✅ 已完成 v1.2.0 |
| ~~F-17~~ | ~~错题专项练习~~ | ~~只练之前答错的题，直到全部正确"出狱"~~ | ✅ 已完成 v1.1.0 |
| F-18 | **艾宾浩斯复习** | 根据遗忘曲线安排复习计划：1d/3d/7d/15d 定时提醒 | 3d |
| ~~F-19~~ | ~~进度统计图表~~ | ~~按天/周/月展示正确率趋势图（ECharts/Recharts）~~ | ✅ 已完成 v1.3.0（Recharts 折线图） |
...
| 编号 | 功能 | 说明 | 预期工作量 |
|------|------|------|-----------|
| ~~F-21~~ | ~~题干搜索~~ | ~~在题库中搜索题干关键词快速定位题目~~ | ✅ 已完成 v1.1.0 |
| F-22 | **批量删除/编辑** | 多选题目后批量删除或批量编辑 | 2d |
| F-23 | **题目收藏** | 收藏重点/难点题目专项复习 | 1d |
| ~~F-24~~ | ~~深色模式~~ | ~~跟随系统或手动切换深色主题~~ | ✅ 已完成 v1.1.4 |
...
|------|-----|
| 项目路径 | `E:\\quiz-app` |
| 域名 | `https://[redacted-pages-project].pages.dev` |
...
| 事项 | 说明 |
|------|------|
| **项目名 ≠ 域名** | CF Pages 项目名是 `quiz-app`，域名是 `[redacted-pages-project].pages.dev`。部署时 `--project-name=quiz-app` |
...
A: 检查 `--project-name` — 是 `quiz-app` 不是 `[redacted-pages-project]`。
