...
本文档描述 5.1.0 的当前实现。历史 V3/V4 方案和旧修复 SQL 仅作追溯，不再作为运行时事实来源。
...
| 本地数据 | Dexie 4、IndexedDB schema v7 |
...
数据库定义在 `src/db.ts`，名称为 `QuizApp`，当前版本为 v7。

| 表 | 主键与索引 | 用途 |
| --- | --- | --- |
| `banks` | `++id, userId, name, createdAt` | 当前账号显式保存的本地题库和离线题库。 |
| `cloudBankCache` | `key, userId, bankId, cachedAt, validatedAt` | 按账号隔离的云端题库加速缓存。 |
...
v7 新增 `cloudBankCache`，主键为 `userId::bankId`。缓存只用于加速云端题库读取，不替代用户主动保存的 `banks/questions` 离线数据。

- 内存缓存有效期 5 分钟。
- IndexedDB 缓存最长保留 7 天。
- 后台版本校验间隔 10 分钟。
- 校验只读取 `content_hash` 与 `question_count`；版本变化后才重新下载完整题目。
- 网络返回后异步写入 IndexedDB，首屏不等待缓存落盘。
- 退出登录清理当前账号缓存，“清除本地数据”清理全部本地缓存。
...
配置：`vite.config.ts`、`sw-custom.js`、`PwaUpdatePrompt.tsx`、`src/utils/pwaManualUpdate.ts`。
...
- 其他安全页面可自动切换；“我的 → 检查更新”会立即调用 `ServiceWorkerRegistration.update()`。
- 发现 waiting 或 installing worker 后发送 `SKIP_WAITING`，由现有 `controllerchange` 监听自动重载页面。

PWA 看到旧 UI 时优先使用“我的 → 检查更新”或自动更新提示；仍未更新再关闭全部 PWA 页面重开或强刷。
...
- OCR 只在拍照搜题时加载。
- 云端题库首次打开受网络、服务区域和题库大小影响；后续打开使用 stale-while-revalidate 风格的账号级持久缓存。
- 首页在题库入口 hover/focus/touch 时预取路由与题库数据；同一请求会合并，避免重复下载。
- 题库未变化时后台只验证元数据；内容哈希变化后才刷新完整题目。
...
- Vite/PWA 生产构建：通过。
- Playwright：5.0.0 桌面 1440 和 Pixel 7 全量基线通过。
- 5.1.0 定向浏览器回归：题库持久缓存与 Profile 检查更新通过；无业务错误，存在已知 Ant Design 静态 message 开发警告。
...
禁止把真实 Token、service role、AI key、固定部署地址或 Pages 项目标识写入源码和 Markdown。旧 V3 文档中的敏感值已脱敏，后续凭据只保存在受控环境。
...
- PWA 自动更新策略变化更新 `pwaUpdateStrategy.test.ts`；手动检查流程更新 `pwaManualUpdate.test.ts`。
...
- `docs/delivery-doc.md`：5.1 交付范围与验收。
