...
当前版本：`5.1.0`
发布状态：5.1.0 预览候选版；5.0.0 已作为生产基线，5.1.0 尚待生产发布确认。
...
5.1.0 基于 5.0.0 生产基线继续优化题库加载与 PWA 更新体验，并保留上一大版本已经完成的视觉、协作、数据一致性和安全能力。

- 新增按账号隔离的云端题库持久缓存，缓存命中后先渲染、后台再校验内容版本，减少重复进入详情和刷题页的等待。
- “我的”页面新增手动检查更新，安装版 PWA 无需等待定时提示即可立即拉取新 Service Worker。
- 首页题库入口增加路由与数据预取，云端题库未变化时避免重复下载完整题目。
...
| 同步与离线 | Dexie/IndexedDB 按账号持久化题库与进度；缓存优先、后台版本校验；离页 beacon 兜底；已缓存题库可离线练习。 |
| 公告反馈 | 公告一次提醒、公告中心、反馈提交、管理员回复、状态管理和受控删除。 |
| 后台管理 | 系统概览、题库审核、用户管理、刷题统计、公告反馈、操作日志。 |
| PWA | 可安装、Network First 页面策略、Service Worker 更新提示、安全自动更新及“我的”页面手动检查更新。 |
...
| 本地存储 | Dexie 4 / IndexedDB，当前 schema v7 |
| 图表 | Recharts 3 |
...
- 浏览器回归：题库持久缓存与“我的 → 检查更新”交互通过；已知 Ant Design 静态 message 开发警告不影响功能。
...
- 云端题库首次打开仍需访问远端服务；后续打开会优先读取本地持久缓存，并在后台校验内容版本。
- Ant Design、导入器和 OCR chunk 会触发构建体积提示，不阻塞 5.1.0。
